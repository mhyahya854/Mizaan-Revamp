...
import { Brackets, Link2 } from "lucide-react";
...
export function PageWikiLinkContext({
  title,
  items,
  direction,
}: {
  title: string;
  items: PageWorkspaceModel["wikiOutgoingLinks"];
  direction: "source" | "target";
}) {
  return (
    <section>
          ...
          return (
            <li key={entry.id}>
              <Link
                ...
                className="flex min-w-0 items-center gap-2 rounded-sm px-2 py-1.5 text-[12.5px] hover:bg-muted"
              >
                <Brackets className="h-3.5 w-3.5 shrink-0 text-faint" />
                <span className="min-w-0 flex-1 truncate">{item.title}</span>
                <span className="shrink-0 text-[11px] text-faint">Wiki</span>
              </Link>
            </li>
          );
        })}
