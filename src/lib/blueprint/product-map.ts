...
export const productModules: ProductModule[] = [
  ...
  },
  {
    ...
    currentTruth:
      "Task metadata, provider-backed creation, dedicated Tasks route/list, board UI, timeline UI, recurrence metadata, reminder metadata, dependency metadata, calendar-link metadata, project-linked editing, task page metadata, search metadata, graph edges, and templates exist. Saved task views, recurrence generation, reminder alarms, native notifications, dependency scheduling, and calendar scheduling are future.",
    futureReason:
      "Saved task views, recurrence generation, reminder alarms, and scheduling should wait until project, people, finance, and calendar relations are stronger.",
    nextPhase:
      "Saved task views, recurrence generation, reminder alarms, and scheduling after relation targets mature.",
  },
  {
    id: "people",
    label: "People",
  ...
  },
  {
    ...
    status: "partial",
    route: "/import-export",
    summary: "Browser archive import manager with JSON paste/load, validation, and preview.",
    currentTruth:
      "Import / Export route exists for Mizaan browser archive JSON only. Native file, folder, markdown, CSV, PDF, SQLite, encrypted, and portable vault imports are not implemented.",
    futureReason: "Full import work needs native filesystem and module-specific format rules.",
    nextPhase: "Future native import design after storage architecture.",
  },
  {
    id: "exports",
    label: "Exports",
    ...
    status: "partial",
    route: "/import-export",
    summary: "Browser archive JSON export manager for the current provider snapshot.",
    currentTruth:
      "Browser archive JSON export exists in Settings/Vault and the Import / Export route. Selected-data, markdown, CSV, PDF, native backup, SQLite backup, encrypted backup, and portable vault export are not implemented.",
    futureReason: "Non-archive export work needs scoped format and validation design.",
    nextPhase: "Future readable export formats after archive policy matures.",
  },
  {
    id: "backups",
    label: "Backups",
  ...
  },
  {
    ...
    status: "partial",
    route: "/repair",
    summary: "Browser prototype data health checks and archive recovery controls.",
    currentTruth:
      "Repair / Recovery route exists with provider counts, category counts, duplicate ID checks, orphan checks, metadata-reference checks, archive validation, restore preview, safe merge, and guarded replace. Automatic repair, migration rollback, native recovery, and encrypted recovery are not implemented.",
    futureReason: "Automatic repair needs a broader issue registry, migration manifest, and logs.",
    nextPhase: "Repair rule expansion after storage architecture.",
  },
  {
    id: "migration-center",
