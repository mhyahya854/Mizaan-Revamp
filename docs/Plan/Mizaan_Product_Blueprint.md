...
- Not implemented yet: task database engine, saved task views, full Gantt engine, recurrence generation, reminder alarms, native notifications, calendar event creation, calendar scheduling automation, dependency engine, AI planning, team/collaboration, and mobile capture.
...
- Error states: generic page errors.
- Actions: create linked or unlinked task, edit task title/status/priority/due date/notes/recurrence metadata/reminder metadata/calendar-link metadata, drag tasks between board status columns, view bounded timeline rows from existing dates, mark done through status, open task page, and search task metadata.
- Disabled/future actions: recurrence generation, reminder scheduling, native notifications, calendar scheduling, dependencies, AI planning, and mobile capture.
