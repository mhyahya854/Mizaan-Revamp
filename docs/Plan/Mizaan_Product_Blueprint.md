...
- Search: [PARTIAL] - provider-backed title, summary, status, category, type, tag, property, metadata, block search, filters, and saved search presets exist, including project/task, people/interaction, and finance metadata; native indexes, OCR, extracted file text, and advanced compound conditions do not.
...
- Tasks: [PARTIAL] - typed task metadata helper, provider-backed task record creation, dedicated `/tasks` route/list, board, bounded timeline UI, recurrence metadata, reminder metadata, dependency metadata, calendar-link metadata, drag/drop status edits, project-linked task creation/editing, task detail metadata panel, metadata search, template defaults, and graph edges are implemented for the browser prototype. There is no task database engine, saved task view model, full Gantt engine, recurrence generation engine, reminder alarm engine, native notification path, dependency scheduling engine, mobile capture, calendar event creation, or calendar-linked scheduling.
...
- Graph: [IMPLEMENTED FOUNDATION] - graph interactive selection, graph filters, local focus view, draggable layout (in-session), explicit relations, parent hierarchy edges, orphan summaries, and open-node actions are implemented for the browser prototype. Persistent graph layout, manual graph/canvas, custom nodes/arrows, wiki-link backlink index, clustering, export, and AI graph are not implemented.
...
- Tasks - provider-backed task records linked to projects with a dedicated browser route; richer task views remain future.
...
| Core          | Search                           | [PARTIAL]                          | Route exists with filters and saved search presets                                                              | Search helper plus saved-search helper                                           | provider-backed browser prototype                                   | Unit tests and browser QA exist                                                       | No native index, advanced compound conditions, OCR, or extracted document text                                                 | Advanced filters and native indexing after provider boundaries      |
...
| Pages         | Tasks                            | [PARTIAL]                          | Dedicated `/tasks` route/list, board, timeline, recurrence metadata, reminder metadata, dependency metadata, calendar-link metadata, project-linked task section, and task metadata panel exist | Typed task metadata in item metadata                                             | localStorage prototype                                              | Helper/search/graph/template/product-map tests and browser QA                       | No task database engine, saved task views, full Gantt, recurrence generation, reminder alarms, notifications, dependency scheduling, calendar event creation, or scheduling | Saved task views and scheduling after relation targets mature       |
...
- Not implemented yet: native index persistence, extracted document text, OCR text, and advanced compound conditions.
...
- Current status: [PARTIAL]
- UI now: dedicated `/tasks` route with list, board, timeline, recurrence metadata filters, reminder metadata stats/preset, dependency metadata stats/readouts, calendar-link metadata stats/readouts, drag/drop status editing on the board, project-linked task creation/list/editing inside project detail metadata, plus a task metadata panel when opening a task page.
- Implemented now: typed task metadata helper, provider-backed task record creation, status/priority/due date/notes/recurrence metadata/reminder metadata/dependency metadata/calendar-link metadata editing, status board grouping, bounded timeline entries from existing task dates, project relation, task template defaults, metadata search, graph edges, and project task counts.
- Not implemented yet: task database engine, saved task views, full Gantt engine, recurrence generation, reminder alarms, native notifications, calendar event creation, calendar scheduling automation, dependency scheduling engine, AI planning, team/collaboration, and mobile capture.
- Data model: provider-backed `MizaanItem` with `category: "tasks"`, `type: "task"`, and normalized task metadata in `item.metadata`.
- Routes: `/tasks`, `/page/$id` for task record pages.
- Main components: dedicated Tasks route, `ProjectMetadataPanel`, `TaskMetadataPanel`, task helper module, and graph/search/template integration.
- Empty states: projects show an honest empty linked-task state with a real New task action.
- Error states: generic page errors.
- Actions: create linked or unlinked task, edit task title/status/priority/due date/notes/recurrence metadata/reminder metadata/dependency metadata/calendar-link metadata, drag tasks between board status columns, view bounded timeline rows from existing dates, mark done through status, open task page, and search task metadata.
- Disabled/future actions: recurrence generation, reminder scheduling, native notifications, calendar scheduling, dependency scheduling, AI planning, and mobile capture.
...
- Done criteria: scoped task route foundation is implemented and verified in the browser/localStorage prototype; overall Tasks remains partial until richer task views and scheduling/reminder systems exist.
- Future phases: richer task views, dependency modeling, and scheduling after relation targets mature.
...
| `/search`        | Search local records  | [PARTIAL]         | Search/filter results and saved search presets | Provider snapshot and saved-search records | Query/filter/open/save/apply/trash presets                                | Native index, OCR/extracted text, advanced conditions | Accurate results and no remote calls        | Search helper tests and QA   |
...
| `/settings`      | Settings/status       | [HARDENED]         | Theme/prototype facts               | Theme/local/provider state             | Theme changes                                                             | Vault config/privacy                             | Safe preferences persist                    | Theme tests and QA           |
