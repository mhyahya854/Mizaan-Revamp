...
- Tasks: [PARTIAL] - typed task metadata helper, provider-backed task record creation, dedicated `/tasks` route/list and board UI, drag/drop status edits, project-linked task creation/editing, task detail metadata panel, metadata search, template defaults, and graph edges are implemented for the browser prototype. There is no task database engine, saved task view model, recurring task engine, reminder engine, native notification path, dependency engine, mobile capture, or calendar-linked scheduling.
...
| Pages         | Tasks                            | [PARTIAL]                          | Dedicated `/tasks` route/list and board plus project-linked task section and task metadata panel exist            | Typed task metadata in item metadata                                             | localStorage prototype                                              | Helper/search/graph/template/product-map tests and browser QA                       | No task database engine, saved task views, recurrence, reminders, notifications, dependencies, or calendar scheduling       | Saved task views and scheduling after relation targets mature       |
...
- Current status: [PARTIAL]
- UI now: dedicated `/tasks` route with list and board modes, drag/drop status editing on the board, project-linked task creation/list/editing inside project detail metadata, plus a task metadata panel when opening a task page.
- Implemented now: typed task metadata helper, provider-backed task record creation, status/priority/due date/notes editing, status board grouping, project relation, task template defaults, metadata search, graph edges, and project task counts.
- Not implemented yet: task database engine, saved task views, recurring tasks, reminders, native notifications, calendar scheduling automation, dependency engine, AI planning, team/collaboration, and mobile capture.
...
- Error states: generic page errors.
- Actions: create linked or unlinked task, edit task title/status/priority/due date/notes, drag tasks between board status columns, mark done through status, open task page, and search task metadata.
