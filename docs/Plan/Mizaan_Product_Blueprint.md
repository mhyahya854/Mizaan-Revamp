...
- Tasks: [PARTIAL] - typed task metadata helper, provider-backed task record creation, dedicated `/tasks` route/list, board, and bounded timeline UI, drag/drop status edits, project-linked task creation/editing, task detail metadata panel, metadata search, template defaults, and graph edges are implemented for the browser prototype. There is no task database engine, saved task view model, full Gantt engine, recurring task engine, reminder engine, native notification path, dependency engine, mobile capture, or calendar-linked scheduling.
...
- Current status: [PARTIAL]
- UI now: dedicated `/tasks` route with list, board, and timeline modes, drag/drop status editing on the board, project-linked task creation/list/editing inside project detail metadata, plus a task metadata panel when opening a task page.
- Implemented now: typed task metadata helper, provider-backed task record creation, status/priority/due date/notes editing, status board grouping, bounded timeline entries from existing task dates, project relation, task template defaults, metadata search, graph edges, and project task counts.
- Not implemented yet: task database engine, saved task views, full Gantt engine, recurring tasks, reminders, native notifications, calendar scheduling automation, dependency engine, AI planning, team/collaboration, and mobile capture.
...
- Error states: generic page errors.
- Actions: create linked or unlinked task, edit task title/status/priority/due date/notes, drag tasks between board status columns, view bounded timeline rows from existing dates, mark done through status, open task page, and search task metadata.
