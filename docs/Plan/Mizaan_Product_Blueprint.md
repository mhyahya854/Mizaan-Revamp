...
- Current status: [PARTIAL]
- UI now: dedicated `/tasks` route with list, board, timeline, and recurrence metadata filters, drag/drop status editing on the board, project-linked task creation/list/editing inside project detail metadata, plus a task metadata panel when opening a task page.
...
- Not implemented yet: task database engine, saved task views, full Gantt engine, recurrence generation, reminders, native notifications, calendar scheduling automation, dependency engine, AI planning, team/collaboration, and mobile capture.
...
- Error states: generic page errors.
- Actions: create linked or unlinked task, edit task title/status/priority/due date/notes/recurrence metadata, drag tasks between board status columns, view bounded timeline rows from existing dates, mark done through status, open task page, and search task metadata.
