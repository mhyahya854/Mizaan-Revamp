...
| Pages         | Tasks                            | [PARTIAL]                          | Dedicated `/tasks` route/list, board, timeline, recurrence metadata, reminder metadata, project-linked task section, and task metadata panel exist | Typed task metadata in item metadata                                             | localStorage prototype                                              | Helper/search/graph/template/product-map tests and browser QA                       | No task database engine, saved task views, full Gantt, recurrence generation, reminder alarms, notifications, dependencies, or scheduling | Saved task views and scheduling after relation targets mature       |
...
- Current status: [PARTIAL]
- UI now: dedicated `/tasks` route with list, board, timeline, recurrence metadata filters, reminder metadata stats/preset, drag/drop status editing on the board, project-linked task creation/list/editing inside project detail metadata, plus a task metadata panel when opening a task page.
- Implemented now: typed task metadata helper, provider-backed task record creation, status/priority/due date/notes/recurrence metadata/reminder metadata editing, status board grouping, bounded timeline entries from existing task dates, project relation, task template defaults, metadata search, graph edges, and project task counts.
- Not implemented yet: task database engine, saved task views, full Gantt engine, recurrence generation, reminder alarms, native notifications, calendar scheduling automation, dependency engine, AI planning, team/collaboration, and mobile capture.
...
- Error states: generic page errors.
- Actions: create linked or unlinked task, edit task title/status/priority/due date/notes/recurrence metadata/reminder metadata, drag tasks between board status columns, view bounded timeline rows from existing dates, mark done through status, open task page, and search task metadata.
