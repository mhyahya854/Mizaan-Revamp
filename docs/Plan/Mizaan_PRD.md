...
| Sidebar/navigation | Partial | Core nav, system tools, pinned pages, page tree, duplicate, trash, pin/unpin, and child creation exist. |
...
| Databases/tables | Partial | Basic provider-backed table model, row/column/cell editing, stats, empty states, and validation helpers exist. No full database engine. |
...
| Projects/tasks | Partial | Typed project and task metadata, project route/list, task records, linked task editing, detail panels, templates, search, and graph edges exist. No dedicated `/tasks` route, recurrence, reminders, notifications, dependencies, kanban, or Gantt. |
...
| Settings | Partial | Theme controls, provider facts, archive panel, and current limitations. Most settings remain read-only prototype facts. |
