...
| Projects/tasks                       | Partial                            | Typed project/task metadata, Projects/Tasks Kanban status tracking views with drag-and-drop, project route/list, dedicated `/tasks` route/list and board UI, task records, linked task editing, templates, and graph edges exist. No saved task views, recurrence, reminders, dependencies, calendar scheduling, or Gantt.                                                        |
...
- **What works now:** Users can view provider-backed task records, use the default status board, drag tasks between status columns, search task metadata, filter by status and priority, create unlinked task records, and edit task status/priority inline.
- **Implementation detail:** Route statistics use the pure `computeTaskTotals` helper, and the board uses the pure `groupTaskRecordsByStatus` helper; `/tasks` is in the generated route tree and browser QA route list.
